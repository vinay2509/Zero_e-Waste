...
function Admincard({ request }) {
  ...
  const supplierPage = () => {
    swal.fire("taking to supplier page", "Assigning Agent", "Loading").then(function () {
      ...
      <div class="card mb-4">
        ...
        <div className="card card-bg">
          ...
          <div className="view overlay">
            {imageload?<img className="card-img-top" src={imageseturl} alt="Card image cap" />:""}
            <a href="#!">
          ...
          <div className="card-body">
            ...
            <h4 className="card-text" style={{ color: "black" }}>{request.name}</h4>
            <h4 style={{ color: "black" }}>{request.email}</h4>


            <p className="card-text bt" style={{ color: "black" }}>Request ID:{request.reqid}</p>
            <p className="card-text bt">City:{request.city}</p>

            <p className="card-text bt" style={{ color: "black" }}>
              {readMore ? request.ewasteQty : `${request.ewasteQty.substring(0, 5)}...`}
              <button onClick={() => setReadMore(!readMore)} className="btn btn-light-blue btn-md">
                {readMore ? 'show less' : '  read more'}
              </button>
            </p>
            <button className="btn btn-danger btn-sm mr-2" onClick={handleDelete}>delete</button>
            {imageload ? '' : (<button onClick={fetchImages} className="btn btn-warning btn-sm mr-2">Show Image</button>)}
            <button className="btn btn-success btn-sm" onClick={supplierPage}>assign agent</button>

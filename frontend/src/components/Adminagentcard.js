...
function Adminagentcard({ agent }) {
  ...
  const setStatus=()=>{
    ...
    // axios.get(`${base_url}/setStatus/${agent.id}/${request_id}`)
    ...
    swal
      ...
      .then((result) => {
        ...
        if (result.isConfirmed) {
          ...
          swal
            .fire("Agent Assigned !", "Changed", "success")
            .then(function () {
  ...
  return (
    <tr>
      ...
      <td>
        {
        agent._free ? 
        <button type="button" class="btn btn-outline-danger btn-sm badge-pill" onClick={setworkstatus}>Assign Agent</button>
        :
        <span>Busy</span>
        }
